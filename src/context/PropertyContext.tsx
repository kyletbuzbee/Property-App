...
interface PropertyContextType {
  ...
  addProperty: (
    property: Omit<PropertyBase, "id" | "createdAt" | "updatedAt">,
  ) => Promise<void>;
...
export function PropertyProvider({
  ...
  const [properties, setProperties] = useState<PropertyWithCalculations[]>(
    initialData ?? [],
  );
  ...
  const fetchProperties = useCallback(
    async (currentFilters: PropertyFilters) => {
      try {
        setLoading(true);
        setError(null);
        ...
        if (currentFilters.minPrice > 0) {
          fetchedProperties = fetchedProperties.filter(
            (p) => p.listPrice >= currentFilters.minPrice,
          );
        }
        if (currentFilters.maxPrice < Infinity) {
          fetchedProperties = fetchedProperties.filter(
            (p) => p.listPrice <= currentFilters.maxPrice,
          );
        }
        if (currentFilters.minEquity > 0) {
          fetchedProperties = fetchedProperties.filter(
            (p) => p.equityGap >= currentFilters.minEquity,
          );
        }
  ...
  const updateProperty = useCallback(
    async (id: string, data: Partial<PropertyBase>) => {
      try {
  ...
  const addProperty = useCallback(
    async (
      propertyData: Omit<PropertyBase, "id" | "createdAt" | "updatedAt">,
    ) => {
      try {
        const response = await fetch("/api/properties", {
          method: "POST",
  ...
  const getPropertiesByStrategy = useCallback(
    (strategy: Strategy) => {
  ...
  const getPropertiesByDecision = useCallback(
    (decision: Decision) => {
