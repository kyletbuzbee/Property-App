...
export default function ExpenseTracker({ budget }: { budget: number }) {
  ...
  const addExpense = () => {
    ...
    setExpenses([
      ...expenses,
      { id: Date.now().toString(), category, amount: Number(amount) },
    ]);
    setAmount("");
  };
  ...
  return (
    <div className="bg-dark-800 p-6 rounded-lg border border-dark-700">
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button
          onClick={addExpense}
          className="bg-primary-600 px-4 rounded text-white text-sm"
        >
          Add
        </button>
      </div>
      ...
      <div className="space-y-2">
        {expenses.map((e) => (
          <div
            key={e.id}
            className="flex justify-between text-sm text-dark-300 border-b border-dark-700 pb-1"
          >
