...
function generateMockSchoolData(lat: number, lng: number): GreatSchoolsResponse {
...
function generateMockWalkScore(lat: number, lng: number, address: string): WalkScoreResponse {
  ...
  const walkScore = Math.min(100, Math.max(10, Math.round(30 + (seed % 40) + cityFactor)));
  const transitScore = Math.min(100, Math.max(5, Math.round(10 + (seed % 35) + cityFactor * 0.5)));
  const bikeScore = Math.min(100, Math.max(15, Math.round(35 + (seed % 35) + cityFactor * 0.3)));
...
function generateMockCrimeData(lat: number, lng: number): CrimeDataResponse {
  ...
  return {
    ...
    trend: seed % 3 === 0 ? 'decreasing' : seed % 3 === 1 ? 'stable' : 'increasing',
    categories: {
...
export async function fetchPropertyEnrichment(
  ...
  if (schools && walkScore && crime) {
    neighborhoodScore = Math.round(
      ...
      walkScore.walkScore * 0.30 + // Walkability 30%
    ...
    neighborhoodScore = totalWeight > 0 ? Math.round(weightedSum / totalWeight) : 50;
  }
...
export function enrichmentToNeighborhoodScore(
  ...
  return {
    schoolsScore: enrichment.schools ? enrichment.schools.overallRating * 10 : undefined,
    crimeScore: enrichment.crime ? 100 - enrichment.crime.crimeIndex : undefined,
